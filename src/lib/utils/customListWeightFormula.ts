        ...
        WEIGHT_FORMULA_DISABLED_FUNCTIONS.map((functionName) => [
            ...
            () => {
                throw new WeightFormulaPreviewError(
                    `Formula function "${functionName}" is disabled`
                );
            }
        ])
    ),
...
function normalizeWeightFormulaResult(result: unknown, formulaName: string): number {
    ...
    if (
        typeof result === 'boolean' || typeof result === 'string' || Array.isArray(result)
        || result == null
    ) {
    ...
    try {
        ...
        throw new WeightFormulaPreviewError(
            error instanceof Error
                ? error.message
                : `${formulaName} must evaluate to a finite number`
        );
    }
}

function sanitizePreviewNumber(
    value: unknown,
    label: string,
...
function normalizePreviewScope(scope: PreviewCustomListWeightFormulaScope) {
    ...
    const normalizedBaseTime = sanitizePreviewNumber(
        scope.baseTime ?? scope.minProgress,
        'baseTime',
        {
            min: 0
        }
    ...
    return {
        ...
        levelCount: sanitizePreviewNumber(scope.levelCount, 'levelCount', {
            integer: true,
            min: 1
        }),
...
export function previewCustomListWeightFormula(
    ...
    return createPreviewCustomListWeightFormula(formula, options)
        .evaluate(scope);
}
