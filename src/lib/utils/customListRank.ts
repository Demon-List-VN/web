...
export function resolveCustomListRankBadge(
    ...
    for (const rankBadge of rankBadges) {
        const meetsRating = rankBadge.minRating == null
            || (playerScore != null && playerScore >= rankBadge.minRating);
        const meetsTop = rankBadge.minTop == null
            || (playerRank != null && playerRank <= rankBadge.minTop);
