	...
	async function fetchData() {
		...
		try {
			tmp.deathCount = await (
				await fetch(
					`${import.meta.env.VITE_API_URL}/deathCount/${uid}/${levelID}`
				)
			).json();
		} catch {
			tmp.deathCount = Array(100)
				.fill(0);
		}
  ...
  <Dialog.Content>
      ...
      {#if record}
        ...
        <Tabs.Root value={selectedTab} class="w-100">
          <Tabs.List>
            ...
            <Tabs.Trigger value="deathCount">{
              $_('record_detail.tabs.death_count')
            }</Tabs.Trigger>
    ...
    && record.data.needMod == false}
              <Tabs.Trigger value="review">{
                $_('record_detail.tabs.review')
              }</Tabs.Trigger>
            {/if}
            {#if $user.loggedIn && $user.data.isAdmin}
              <Tabs.Trigger value="edit">{
                $_('record_detail.tabs.edit')
              }</Tabs.Trigger>
            {/if}
          </Tabs.List>
    ...
    && $user.data.uid == record.data.players.uid}
                      <Dialog.Root bind:open={open1}>
                        ...
                        <Dialog.Content>
                          ...
                          <Button bind:disable={disableBtn} on:click={change}
                          >Change</Button>
                        </Dialog.Content>
                      </Dialog.Root>
                    {/if}
            ...
            {#if skipAheadState == 0}
              <div class="flex flex-col gap-[15px]">
                ...
                <div class="flex items-center justify-between">
                  <p class="text-lg">
                    <b>{$_('record_detail.skip_ahead.total')}</b>
                  </p>
                  <p class="text-lg">
                    <b>{formatPrice(5000 * daysToSkip[0])}₫</b>
                  </p>
                </div>
          ...
          <Tabs.Content value="review">
            <RadioGroup.Root bind:value={verdict}>
              ...
              <div class="flex items-center space-x-2">
                ...
                >Need further inspection (This record will be forwarded to a
                  moderator).</Label>
              </div>
              <Textarea
                bind:value={cmt}
