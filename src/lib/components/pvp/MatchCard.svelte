<script lang="ts">
	...
	$: shouldShowLevel = status !== 'ban_pick'
		&& (!hideLevelUntilConfirmed || isPvpMatchConfirmedByBoth(match));
	...
	function resultLabel() {
		...
		if (status === 'pending') {
			return `${$_('pvp.awaiting_acceptance_short')} ${
	...
	function participantName(participant: typeof titleLeft) {
		...
		if (shouldHideParticipantInfo(participant)) {
	...
	function shouldMaskParticipant(participant: typeof titleLeft) {
		return participantIsAnonymous(participant)
