<script lang="ts">
	...
	$: profileBannerUrl = $user.loggedIn && isSupporter && $user.data?.uid
		? `https://cdn.gdlisthub.dev/banners/${$user.data.uid}${
			$user.data.isBannerGif ? '.gif' : '.jpg'
		}?version=${$user.data.bannerVersion || 0}`
		: null;
...
<div class="heroBanner" class:hasBannerBg={!!profileBannerUrl && !bannerError}>
  ...
  <div class="heroContent">
    ...
    {:else}
      ...
      <h1 class="heroTitle">
        {
          $_('homepage.hero.welcome_back', { values: { name: $user.data.name } })
        } 💖
      </h1>
      <p class="heroSubtitle">
        {$_('homepage.hero.supporter_welcome_subtitle')}
      </p>
      <div class="heroCtas">
        <Button
          href={`/@${$user.data.name}`}
          variant="outline"
          class="heroBtn secondaryBtn"
        >
