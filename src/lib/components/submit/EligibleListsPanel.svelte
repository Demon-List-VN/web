<script lang="ts">
	...
	$: visibleLists = mode === 'eligible'
		? lists.filter((list) => Boolean(list.eligible))
		: lists;
	$: panelTitle = mode === 'eligible'
		? t('Danh sách đủ điều kiện', 'Eligible lists')
		: t('Các list có level này', 'Lists containing this level');
	$: panelDescription = mode === 'eligible'
		? t(
	...
	$: loadingMessage = mode === 'eligible'
		? t('Đang kiểm tra list phù hợp...', 'Checking eligible lists...')
		: t(
			'Đang tải các list chứa level này...',
			'Loading lists containing this level...'
		);
	$: emptyMessage = mode === 'eligible'
		? t(
			'Chưa có list nào phù hợp với record hiện tại.',
			'No lists match the current record yet.'
		)
		: t(
			'Level này chưa nằm trong list công khai nào.',
			'This level is not in any public list yet.'
		);
</script>
      ...
      {#each visibleLists as list}
        ...
        >
          <div class="eligible-list-top">
            ...
            <span class="eligible-list-link-icon"><ExternalLink
                size={14}
              /></span>
          </div>
