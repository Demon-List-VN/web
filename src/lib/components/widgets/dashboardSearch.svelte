...
{#if searchEnabled}
  <form
    ...
    class="flex w-full flex-col items-center gap-3 sm:gap-4"
        ...
        {#each shortcuts as shortcut}
          ...
          >
            {#if shortcut.icon}
              ...
              />
            {:else}
              <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-muted text-sm font-bold sm:h-10 sm:w-10 sm:text-base">
                {shortcut.name.charAt(0)
.toUpperCase()}
              </div>
            {/if}
            <span
              class="max-w-[60px] truncate text-[10px] text-muted-foreground sm:max-w-[80px] sm:text-xs"
            >{shortcut.name}</span>
          </a>
        {/each}
      </div>
