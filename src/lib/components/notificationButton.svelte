<script lang="ts">
	...
	function mergeNotification(notification: UserNotification) {
		if (notification.metadata?.type === 'social_message') {
			return;
		}
		...
		notifications = [
			normalized,
			...notifications.filter((item) => notificationKey(item) !== key)
		];
	}

	async function handleSocialNotification(
		uid: string,
		notification: UserNotification
	) {
		if (notification.metadata?.type !== 'social_message') {
		...
		try {
			await handleSocialMessageNotification(
				uid,
				await $user.token(),
	...
	async function deleteNotification(notification: UserNotification) {
		...
		const response = await fetch(
			...
			}/${encodeURIComponent(String(notification.id))}`,
			{
				method: 'DELETE',
				headers: {
	...
	function timeSince(date: any) {
		...
		for (const { unit, seconds: intervalSeconds } of intervals) {
			...
			if (count >= 1) {
				return count + ' ' + $_(`time_unit.${unit}`)
					+ ($locale === 'en' && count > 1 ? 's' : '');
			}
		}

		return $_('time_unit.now')
			.charAt(0)
			.toUpperCase()
			+ $_('time_unit.now')
				.slice(1);
	...
	async function fetchNotifications(uid: string) {
		...
		try {
			...
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/notifications/${uid}`,
				{
					headers: {
						Authorization: 'Bearer ' + token
					}
			...
			notifications = Array.isArray(data)
				? data
					...
					.filter((notification) =>
						notification.metadata?.type !== 'social_message'
					)
				: [];
		} catch {
			notifications = [];
	...
	async function clear() {
		if (!$user.loggedIn || !$user.data?.uid || clearing) {
			return;
		}
		...
		try {
			await fetch(
				`${import.meta.env.VITE_API_URL}/notifications/${$user.data.uid}`,
				{
					method: 'DELETE',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())!
