<script lang="ts">
	...
	function getDefaultGraphXAxis(
		platformer: boolean,
		type: 'recordScore' | 'weight'
	) {
	...
	function getFormulaVariables(type: 'recordScore' | 'weight') {
		...
		return [
			{
				token: 'score',
				label: $_('custom_lists.formula.score_label'),
				inputKey: 'score' as const
			},
			{
				token: 'position',
	...
	async function fetchFormulaOutput(
		input: PreviewInput,
	...
	async function runPreview() {
		...
		try {
			...
			const evaluator = createPreviewCustomListWeightFormula(
				formula,
				getFormulaPreviewOptions()
			);
	...
	$: if (
		isPlatformer !== previousPlatformer || formulaType !== previousFormulaType
	) {
	...
	$: if (
		...
		graphXAxis = formulaVariables.find((variable) =>
			variable.token === getDefaultGraphXAxis(isPlatformer, formulaType)
		)?.token
...
<div class="formulaPreviewCard">
  ...
  <div class="formulaPreviewGrid">
    {#each formulaVariables as variable}
      <div class="field">
        <label for={`formula-preview-${formulaType}-${variable.token}`}>{
          variable.label
        }</label>
        <Input
  ...
  >
    ...
    {:else if previewRecordPoint != null}
      <span class="formulaPreviewResultValue">{
        formatOutput(previewRecordPoint)
      }</span>
    {:else}
    ...
    <div class="formulaPreviewGraphHeader">
      ...
      <div class="formulaPreviewGraphControls">
        <div class="field formulaPreviewGraphAxisField">
          <label for={`formula-preview-${formulaType}-x-axis`}>{
            $_('custom_lists.formula.x_axis_label')
          }</label>
          <select
            id={`formula-preview-${formulaType}-x-axis`}
            bind:value={graphXAxis}
          >
            {#each formulaVariables as variable}
        ...
        <div class="field formulaPreviewGraphRangeField">
          <label for={`formula-preview-${formulaType}-graph-range-min`}>{
        ...
        <div class="field formulaPreviewGraphRangeField">
          <label for={`formula-preview-${formulaType}-graph-range-max`}>{
        ...
        <label
          ...
          for={`formula-preview-${formulaType}-cumulative-sum`}
        >
          <input
      ...
      <div class="formulaPreviewBarChart">
        <div class="formulaPreviewBarChartSummary">
          ...
          <span>{graphOutputLabel}: {
              graphDisplayCurrentPoint ? formatOutput(graphDisplayCurrentPoint.y) : '-'
            }</span>
        </div>
