<script lang="ts">
	...
	let post: any = initialPost;
	let comments: any[] | null = initialComments;
	let newComment = '';
	...
	async function fetchComments() {
		...
		try {
			const res = await fetch(
				`${apiPrefix}/posts/${postId}/comments`,
				{ headers }
			);
	...
	async function toggleLike() {
		...
		try {
			const res = await fetch(`${apiPrefix}/posts/${post.id}/like`, {
				method: 'POST',
				headers
			});
	...
	async function submitComment() {
		...
		try {
			...
			const res = await fetch(
				`${apiPrefix}/posts/${post.id}/comments`,
				{
					method: 'POST',
					headers,
	...
	async function deleteComment(commentId: number) {
		...
		try {
			await fetch(`${apiPrefix}/comments/${commentId}`, {
				method: 'DELETE',
				headers
			});
	...
	async function toggleCommentLike(comment: any) {
		...
		try {
			const res = await fetch(
				`${apiPrefix}/comments/${comment.id}/like`,
				{ method: 'POST', headers }
			);
			const result = await res.json();
	...
	async function deletePost() {
		...
		try {
			await fetch(`${apiPrefix}/posts/${post.id}`, {
				method: 'DELETE',
				headers
			});
	...
	async function togglePin() {
		...
		try {
			const res = await fetch(`${apiPrefix}/posts/${post.id}/pin`, {
				method: 'POST',
				headers
			});
	...
	async function searchMentions(query: string) {
		try {
			const res = await fetch(
				`${apiPrefix}/players/search?q=${encodeURIComponent(query)}`
			);
	...
	onMount(() => {
		if (!initialPost) {
			// Client-side only mode: fetch on mount
			fetchPost();
			fetchComments();
...
<div class="postPage">
	<!-- Back nav -->
	<div class="backNav">
		<a href={backLink} class="backLink">
