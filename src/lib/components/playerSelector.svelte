	...
	async function searchPlayers() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/search/${searchValue}`,
				{
      ...
      {:else if searchValue.length < 2}
        <div class="dropdown-empty">
          ...
          <p class="text-sm text-muted-foreground">
            {$t('playerSelector.type_to_search')}
          </p>
        </div>
      {:else if searchResults.length === 0}
        <div class="dropdown-empty">
          <p class="text-sm text-muted-foreground">
            {$t('playerSelector.no_players_found')}
          </p>
          <p class="text-xs text-muted-foreground">
            {$t('playerSelector.try_different_name')} <code
            >discord:USER_ID</code>
          </p>
        </div>
      {:else}
            ...
            >
              ...
              <div class="player-info">
                ...
                <div class="player-stats">
                  {#if player.rating}
                    <span class="stat">{$t('playerSelector.rating')}: {
                        player.rating
                      }</span>
                  {/if}
                  {#if player.overallRank}
                    <span class="stat">{$t('playerSelector.rank')}: #{
                        player.overallRank
                      }</span>
                  {/if}
                  {#if player.province}
