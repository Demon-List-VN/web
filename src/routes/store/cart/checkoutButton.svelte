  ...
  <Dialog.Content>
    {#if state == 0}
      ...
      <div class="flex flex-col gap-[5px]">
        <Button
          variant="outline"
          class={`h-[70px] justify-start ${
              paymentMethod == 'Bank Transfer' ? 'border-white' : ''
          }`}
          on:click={() => {
    ...
    {:else if state == 1}
      ...
      <div class="grid grid-cols-4 items-center gap-4">
        <Label class="text-right">{
          $_('store.checkout.shipping.address')
        }</Label>
        <Textarea
          class="col-span-3"
      ...
      <div class="grid grid-cols-4 items-center gap-4">
        <Label class="text-right">{
          $_('store.checkout.shipping.recipient_name')
        }</Label>
        <Input
          class="col-span-3"
      ...
      <div class="grid grid-cols-4 items-center gap-4">
        <Label class="text-right">{
          $_('store.checkout.shipping.phone_number')
        }</Label>
...
.format(
                    items.reduce(
                        ...
                        + $cart.getRecordCards()
.reduce(
                            (total, rc) => total + (rc.material === 'paper' ? 29000 : 149000),
                            0
                        )
                )
              }
