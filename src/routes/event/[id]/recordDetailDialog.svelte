<script lang="ts">
	...
	async function loadDeathCountData() {
		...
		try {
			...
			levelDeathCount = levelData.count || Array(100)
				.fill(0);
			playerDeathCount = playerData.count || Array(100)
				.fill(0);
		} catch (error) {
			console.error('Failed to load death count data:', error);
			levelDeathCount = Array(100)
				.fill(0);
			playerDeathCount = Array(100)
				.fill(0);
		} finally {
						...
						callbacks: {
							label: function (context) {
								...
								const count = datasetIndex === 0
									? levelDeathCount[dataIndex]
									: playerDeathCount[dataIndex];

								if (showRate) {
									const rate = context.parsed.y.toFixed(2);

									return `${context.dataset.label}: ${rate}% (${count} ${
										$_('contest.death_count.deaths')
									})`;
								} else {
...
<Dialog.Root>
  ...
  <Dialog.Content
    class={currentTab === 'deathcount' ? 'max-h-[90vh] md:max-w-[80vw]' : ''}
  >
    <Dialog.Header>
    ...
    <Tabs.Root value="detail" class="w-full" onValueChange={handleTabChange}>
      ...
      <Tabs.Content value="detail">
        <div class="flex flex-col gap-0">
          <section>
            <span class="font-bold">{$_('contest.leaderboard.submitted_at')}:
            </span>
            {new Date(record.created_at)
.toLocaleString('vi-vn')}
          </section>
          <section>
            <span class="font-bold">{$_('contest.leaderboard.video_link')}:
            </span><a
          ...
          <section>
            ...
            {#if record.raw}
              <a href={record.raw} class="text-[#95bdf7]" target="_blank"
              >Link</a>
            {:else}
          ...
          {:else if record.accepted === false}
            <section>
              <span class="font-bold">{$_('contest.leaderboard.reject_reason')}:
              </span>
              {record.rejectReason}
            </section>
            <section class="mt-[10px] text-[13px] opacity-50">
            ...
            <div class="grid gap-4 py-4">
              <div class="grid grid-cols-4 items-center gap-4">
                <Label for="name" class="text-right">{
                  $_('contest.leaderboard.submitted_at')
                }</Label>
                <Input
                  type="datetime-local"
              ...
              <div class="grid grid-cols-4 items-center gap-4">
                <Label for="name" class="text-right">{
                  $_('contest.leaderboard.video_link')
                }</Label>
              ...
              <div class="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  bind:value={updateData.rejectReason}
                  class="col-span-3"
                />
              </div>
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <div class="opacity-70">
                  {$_('contest.death_count.total')}: {
                    levelDeathCount.reduce((a, b) => a + b, 0)
                  }
                </div>
              </div>
              <div>
                ...
                <div class="opacity-70">
                  {$_('contest.death_count.total')}: {
                    playerDeathCount.reduce((a, b) => a + b, 0)
                  }
                </div>
              </div>
            </div>
