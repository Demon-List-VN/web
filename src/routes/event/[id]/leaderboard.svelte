<script lang="ts">
	...
	function getPoint(record: any, index: number, checkAcp = false) {
		if (
			record === null || record == 'revealed'
			|| (record.accepted === false && checkAcp)
		) {
			return 0;
		}
	...
	async function refresh(noti = false) {
		const upd = async () => {
			...
			leaderboard = await (
				await fetch(
					`${import.meta.env.VITE_API_URL}/events/${event.id}/leaderboard`,
					{
						method: 'GET',
						headers: {
							Authorization: $user.loggedIn
								? 'Bearer ' + (await $user.token())
								: ''
						}
					}
				)
	...
	function exportToCSV() {
		...
		leaderboard.forEach((player, rank) => {
			const playerName = escapeCSV(
				(player.clan ? `[${player.clans.tag}] ` : '') + player.name
				|| 'Unknown'
			);
			const userID = escapeCSV(player.uid || 'Unknown');

			csvContent += `${rank + 1},${playerName},${userID},${
				getTotalPoint(player.eventRecords)
	...
	function reveal(uid: string, levelIndex: number, rank: number) {
		...
		if (!res.eventRecords[levelIndex]) {
			...
			leaderboard[rank].eventRecords[levelIndex] =
				res.eventRecords[levelIndex];
		}
		...
		leaderboard.sort((a, b) => {
			...
			if (x == y && x != 0) {
				return getPenalty(a.eventRecords, 1)
					- getPenalty(b.eventRecords, 1);
			}
	...
	async function revertCalculation() {
		toast.promise(
			fetch(
				`${import.meta.env.VITE_API_URL}/events/${event.id}/revert-calc`,
				{
					method: 'PATCH',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())
	...
	async function getPlayerDeathCount(levelID: number, userID: string) {
		const res = await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/deathCount/${userID}/${levelID}`
			)
		).json();
...
<div class="mb-[10px] flex justify-center gap-[10px]">
  ...
  {#if $user.loggedIn && $user.data.isAdmin}
    ...
    <Button
      variant="outline"
      disabled={!event.isCalculated}
      on:click={revertCalculation}
    ...
    <Table.Header>
      <Table.Row>
        <Table.Head class="w-[100px]">{
          $_('contest.leaderboard.rank')
        }</Table.Head>
        <Table.Head class="min-w-[200px]">{
          $_('contest.leaderboard.player')
        }</Table.Head>
        <Table.Head class="w-[75px] text-center">{
          $_('contest.leaderboard.total')
        }</Table.Head>
        {#if event.type !== 'basic'}
          <Table.Head class="w-[75px] text-center">{
            $_('contest.leaderboard.penalty')
          }</Table.Head>
        {/if}
      ...
      {#each leaderboard as player, rank (player.uid)}
        ...
        >
          {#if $user.loggedIn && player.uid == $user.data.uid}
            <Table.Cell class="w-[100px] font-medium text-yellow-500">#{
                rank + 1
              }</Table.Cell>
          {:else}
          ...
          {#if event.type !== 'basic'}
            <Table.Cell class="w-[75px] text-center">
              <Tooltip.Root>
                ...
                <Tooltip.Content>{
                  getPenaltyTooltip(player.eventRecords)
                }</Tooltip.Content>
              </Tooltip.Root>
            </Table.Cell>
          {/if}
          ...
          {#if event.type == 'raid' && previousScores.size > 0}
            ...
            <Table.Cell
              class={`w-[75px] text-center ${
                  Math.sign(scoreDiff) > 0
