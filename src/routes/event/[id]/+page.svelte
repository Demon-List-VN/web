...
<svelte:head>
  ...
  <meta
    property="og:title"
    content={`${data.title} - ${$_('head.site_name')}`}
  />
  <meta property="og:description" content={data.description} />
  <meta
    property="og:image"
    content={data.imgUrl ? data.imgUrl : `https://cdn.gdlisthub.dev/event-banner/${data.id}.webp`}
  />
</svelte:head>
...
{#if !data.hidden || ($user?.loggedIn && $user?.data?.isAdmin)}
  <div class="flex items-center justify-center gap-[10px] mt-[10px]">
    ...
    {#if $user?.loggedIn && $user?.data?.isAdmin}
      <Button href="/admin/event?id={data.id}" variant="outline"
      >Manage Event</Button>
    {/if}
  </div>
  {#if data.isContest}
    <Tabs.Root value="detail" class="mt-[20px] flex flex-col items-center">
      ...
      <Tabs.Content
        value="leaderboard"
        class="mt-[20px] w-full pl-[10px] pr-[10px]"
      >
        {#if levels.length}
