<script lang="ts">
	...
	async function fetchItem(itemId: number, inventoryId: number) {
		const inventoryItem = await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/inventory/${inventoryId}`,
				{
					method: 'GET',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())
		...
		try {
			...
			selectedItems = {
				...
				[inventoryId]: {
					loading: false,
					data: null,
					error: get(_)('inventory.failed_load_item')
				}
			};
		}
	}
	...
	function canUseItem(
		itemData:
			| { useRedirect?: string | null; productId?: number | null; }
			| null
			| undefined
	) {
	...
	async function use(
		inventoryId: number,
		fallbackRedirect: string | null = null
	) {
		const consumeRequest = (async () => {
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/inventory/${inventoryId}/consume`,
				{
					method: 'DELETE',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())
			...
			if (result && typeof result === 'object' && 'redirectTo' in result) {
				return typeof result.redirectTo === 'string'
					? result.redirectTo
					: fallbackRedirect;
			}
        ...
        <Dialog.Root>
          <Dialog.Trigger>
            <button
              ...
              class={`relative w-full transform rounded-md border-b-4 bg-white/5 p-0 transition-transform hover:scale-105 ${
            ...
            >
              ...
              {#if item.inventoryQuantity > 1}
                <div class="absolute bottom-2 right-2 z-10 rounded-full bg-black/80 px-2 py-1 text-xs font-bold text-white">
          ...
          <Dialog.Content>
            ...
            {:else if selectedItems[item.inventoryId]?.error}
              <div class="py-4 text-sm text-red-400">
                {selectedItems[item.inventoryId].error}
              </div>
            {:else if selectedItems[item.inventoryId]?.data}
              <div class="flex items-start gap-4">
                <div
                  class={`relative h-40 w-40 flex-shrink-0 overflow-hidden rounded-md border-b-4 ${
                      rarityClass(selectedItems[item.inventoryId].data.rarity ?? item.rarity)
                  } bg-neutral-800 p-2`}
                >
                  <img
                    src={`https://cdn.gdvn.net/items/${item.itemId}.webp`}
                ...
                <div class="h-full flex-1">
                  <div class="flex h-full flex-1 flex-col">
                    <div>
                      <Dialog.Header>
                        <Dialog.Title>{
                          selectedItems[item.inventoryId].data.name ?? item.name
                        }</Dialog.Title>
                        <Dialog.Description>{
...
.toLocaleString('vi-vn')
                    ...
                    <div class="mt-auto">
                      ...
                      {:else if canUseItem(selectedItems[item.inventoryId].data)}
                        <Button
                          ...
                          use(
                              item.inventoryId,
                              selectedItems[item.inventoryId].data.useRedirect ?? null
                          )}
              ...
              {#if item.type == 'case'}
                ...
                {#if selectedItems[item.inventoryId].data.caseItems
    && selectedItems[item.inventoryId].data.caseItems.length > 0}
                  <div class="mt-2 grid grid-cols-3 gap-2">
                    {#each selectedItems[item.inventoryId].data.caseItems as c (c.id)}
                      <div
                        class={`flex flex-col items-center rounded-md border-b-4 bg-neutral-900 p-2 text-center ${
                            rarityClass(c.items.rarity ?? item.rarity)
                        }`}
                      >
                        <img
                ...
                {:else}
                  <div
                    class={`h-40 w-40 flex-shrink-0 overflow-hidden rounded-md border-b-4 ${
                        rarityClass(item.rarity)
