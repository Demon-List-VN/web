<script lang="ts">
	...
	$: isMember = $user.loggedIn && $user.data?.clan == clan.id;

	async function fetchAvailableTags() {
		try {
			const res = await fetch(`${API_PREFIX}/tags`);
	...
	async function fetchPosts(append = false) {
		...
		try {
			...
			if (sortMode === 'recommended') {
				...
				res = await fetch(`${API_PREFIX}/posts/recommended?${params}`, {
					headers
				});
			} else {
				const params = new URLSearchParams({
				...
				res = await fetch(`${API_PREFIX}/posts?${params}`, {
					headers
				});
			}
	...
	function setupIntersectionObserver() {
		const sentinel = document.querySelector('#clanLoadMoreSentinel');
		if (!sentinel) return;
...
<div class="clanCommunity">
	<!-- Toolbar -->
	<div class="toolbar">
		<div class="toolbarTop">
			{#if isMember}
				<a href="/community/create?clanId={clan.id}" class="createBtnLink">
		...
		<div class="toolbarBottom">
			...
			<div class="sortFilters">
				<button
					class="sortBtn recommended"
					class:active={sortMode === 'recommended'}
					on:click={() => switchSort('recommended')}
				>
	...
	<div class="postsGrid">
		{#if posts.length > 0}
			{#each posts as post}
				<CommunityPostCard {post} apiPrefix={API_PREFIX} on:report={handleReport} />
			{/each}
		{:else if !loading}
	...
	<!-- Load More -->
	{#if posts.length > 0}
		<div id="clanLoadMoreSentinel" class="loadMoreSentinel" />
		{#if loading}
