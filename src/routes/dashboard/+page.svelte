<script lang="ts">
	...
	async function getEvents() {
		return await (await fetch(`${import.meta.env.VITE_API_URL}/events/ongoing`))
			.json();
	}
	...
	async function getRecentDemonListLevel() {
		...
		return await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/list/dl?${query.toString()}`
			)
		).json();
	}
	...
	async function getRecentFeaturedListLevel() {
		...
		return await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/list/fl?${query.toString()}`
			)
		).json();
	}
	...
	async function getRecentPlatformerListLevel() {
		...
		return await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/list/pl?${query.toString()}`
			)
		).json();
	}
	...
	onMount(async () => {
		...
		getRecentDemonListLevel()
			.then((data) => (recent.dl = data));
		getRecentFeaturedListLevel()
			.then((data) => (recent.fl = data));
		getRecentPlatformerListLevel()
			.then((data) => (recent.pl = data));
	});
</script>
