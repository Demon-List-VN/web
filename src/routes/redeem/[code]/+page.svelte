<script lang="ts">
	...
	onMount(async () => {
		try {
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/coupon/${page.params.code}`,
				{
					method: 'GET',
					headers: {
	...
	function getTimeLeft(validUntil: string) {
		...
		if (days > 0) {
			...
			return `${hours} ${$_('redeem.hours')} ${minutes} ${
				$_('redeem.minutes')
			}`;
		} else {
...
{#if loading}
  <div
    class="mt-[40px] flex items-center justify-center"
...
{:else if data && data.percent == 1}
  ...
  >
    <Card.Root
      class="mx-auto w-full max-w-[600px] overflow-hidden border-0 shadow-2xl"
    >
      <div class="bg-gradient-to-br from-purple-500 via-pink-500 to-red-500 p-[2px]">
        ...
        >
          <div
            class="
              p-8 text-white transition-opacity duration-700 {showContent
              ? 'opacity-100'
              : 'opacity-0'}
            "
          >
