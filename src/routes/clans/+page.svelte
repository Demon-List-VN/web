<script lang="ts">
	...
	async function fetchClans(append = false) {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/clans?${params}`
			);
	...
	async function fetchMyClan() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/clans/${$user.data.clan}`
			);
	...
	async function createClan() {
		if (
			...
			toast.error(
				'Tag must be alphanumeric, contains no space and must have 2-6 characters.'
			);
	...
	async function boostPurchase() {
		...
		const res: any = await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/payment/getPaymentLink/3/${
					boostQuantity[0]
				}?targetClanID=${$user.data.clan}`,
				{
					method: 'POST',
					headers: {
						Authorization: 'Bearer ' + (await $user.token()),
...
<div class="pageContainer">
  ...
  <div class="pageHeader">
    ...
    <Dialog.Root>
      ...
      <Dialog.Content>
        ...
        <Dialog.Footer>
          <Button on:click={createClan}>{
            $_('clans.create.create_button')
          }</Button>
        </Dialog.Footer>
      </Dialog.Content>
    </Dialog.Root>
  ...
  {#if $user.loggedIn && $user.data.clan}
    ...
    {:else if myClan}
      <a
        href={`/clan/${myClan.id}`}
        class="myClanCard"
        class:boosted={isBoostActive(myClan)}
      >
  ...
  <div class="boostCtaBanner">
    ...
    <div class="boostCtaRight">
      {#if $user.loggedIn && $user.data.clan}
        <p class="boostCtaSelectLabel">
          {$_('clans.boost_cta.select_duration')}
        </p>
        <div class="boostSliderRow">
          <Slider bind:value={boostQuantity} max={30} min={1} step={1} />
          <span class="boostDays">{boostQuantity[0]} {
              $_(boostQuantity[0] > 1 ? 'clan.boost.days_plural' : 'clan.boost.days')
            }</span>
        </div>
        <div class="boostPriceRow">
          <span class="boostPrice">{
              formatPrice(5000 * boostQuantity[0])
            }₫</span>
      ...
      {:else}
        <p class="boostCtaDisabledMsg">
          {$_('clans.boost_cta.login_required')}
        </p>
      {/if}
    </div>
  </div>
      ...
      <div class="clansGrid">
        {#each clans as clan (clan.id)}
          <a href={`/clan/${clan.id}`} class="clanCardLink">
            <Card.Root
              class={`clanCard ${isBoostActive(clan) ? 'clanCardBoosted' : ''}`}
            >
              ...
              <Card.Content class="clanCardContent">
                ...
                <div class="clanCardFooter">
                  <span class="clanMemberCount">
                    ...
                    {clan.memberCount}{#if clan.memberLimit}/{
                        clan.memberLimit
                      }{:else}/∞{/if}
                  </span>
                  {#if clan.rating}
          ...
          <Card.Root class="clanCard">
            ...
            <Card.Content class="clanCardContent">
              ...
              <div class="invitationActions">
                <Button
                  size="sm"
                  on:click={() => acceptInvitation(invitation.clans.id)}
                >
                  {$_('general.accept')}
                </Button>
                <Button
