<script lang="ts">
	...
	onMount(() => {
		document.getElementById(`page${curPage}`)
			?.click();
	});
</script>
...
<svelte:head>
  <title>
    {$_('head.titles.leaderboard')} - {getListTitle()} - {$_('head.site_name')}
  </title>
</svelte:head>
    ...
    <Table.Header>
      <Table.Row>
        ...
        <Table.Head class="w-[70px] text-right">
          {
            $page.params.list == 'dl' || $page.params.list == 'pl'
            || $page.params.list == 'cl'
            ? $_('list.tabs.rating')
        ...
        <Table.Row>
          ...
          <Table.Cell>
            <div class="playerNameWrapper">
              <PlayerLink
                {player}
                showTitle={$page.params.list != 'pl'}
                titleType={$page.params.list}
              />
            </div>
          </Table.Cell>
