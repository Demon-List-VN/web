<script lang="ts">
	...
	function remainingLongLabel(targetMs: number | null, currentNow: number) {
		...
		const totalMinutes = Math.max(
			0,
			Math.floor((targetMs - currentNow) / 60_000)
		);
	...
	function formatWinrate(value: number | null | undefined) {
		...
		return `${
			winrate.toLocaleString($locale || 'en', {
	...
	function isBoostActive(clan: PvpClan | null) {
		...
		const boostedUntilMs = new Date(boostedUntil)
			.getTime();
	...
	function clanTagStyle(clan: PvpClan | null) {
		if (!clan || !isBoostActive(clan) || !clan.tagBgColor) {
			return '';
		}

		return `background-color: ${clan.tagBgColor}; color: ${
  ...
  <Card.Root>
    ...
    <Card.Content>
      <Tabs.Root bind:value={activeTab}>
        <Tabs.List
          class="clan-race-tab-list py-[20px]"
          aria-label={$_('pvp.clan_race.tabs.label')}
        >
        ...
        <Tabs.Content value="standings">
          ...
          {:else}
            {#if userClanRow}
              ...
              <div
                class="clan-race-user-result"
                aria-label={$_('pvp.clan_race.your_result')}
              >
                <span>
                  {#if userClanRow.rank}
                    #{userClanRow.rank}
                ...
                <span class="leaderboard-clan">
                  {#if clan}
                    <a href={`/clan/${clan.id}`} class="clan-link">
                      ...
                      <span class:boosted={isBoostActive(clan)}>{
                        clan.name
                      }</span>
                      {#if clan.tag}
                        <small
            ...
            {:else}
              <div
                ...
                role="table"
                aria-label={$_('pvp.clan_race.title')}
              >
                ...
                {#each clanRace.leaderboard as row}
                  ...
                  <div class="leaderboard-row" role="row">
                    <span class="leaderboard-rank" role="cell">#{
                        row.rank
                      }</span>
