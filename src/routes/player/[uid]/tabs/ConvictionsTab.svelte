	...
	onMount(async () => {
		try {
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/players/${userID}/convictions`
			);
...
{:else}
  <Table.Root>
    <Table.Caption>{$_('player.table.total_convictions')}: {
        convictions.length
      }</Table.Caption>
    <Table.Header>
      <Table.Row>
        ...
        <Table.Head class="w-[120px] text-center">{
          $_('player.convictions.credit_reduce')
        }</Table.Head>
        <Table.Head class="w-[220px] text-center">{
          $_('player.convictions.created_at')
        }</Table.Head>
      </Table.Row>
    </Table.Header>
    <Table.Body>
      {#each convictions as conviction}
        <Table.Row>
          ...
          <Table.Cell class="text-center">
            {new Date(conviction.created_at)
