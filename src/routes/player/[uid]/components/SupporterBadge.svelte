<script lang="ts">
	...
	$: expiryDate = supporterUntil
		? new Date(supporterUntil)
			.toLocaleDateString()
		: '';
</script>
...
{#if isActive(supporterUntil)}
  {#if isOwner}
    <Tooltip.Root>
      <Tooltip.Trigger>
        <Badge
          class="supporter-badge gap-1 border-yellow-500/30 bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20"
        >
