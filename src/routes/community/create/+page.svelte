<script lang="ts">
	...
	async function fetchTags() {
		...
		try {
			...
			availableTags = $user.data?.isAdmin
				? allTags
				: allTags.filter((t: any) => !t.adminOnly);
		} catch {
			availableTags = [];
		} finally {
			loadingTags = false;
	...
	function toggleTag(tagId: number) {
		if (selectedTagIds.includes(tagId)) {
			...
			if (selectedTagIds.length >= 5) {
				toast.error(
					$_('community.create.max_tags') || 'Maximum 5 tags allowed'
				);
	...
	async function prefillListAttachment(listId: string) {
		if (!listId || listId === selectedListKey) {
			return;
		}
	...
	async function fetchMyRecords() {
		...
		try {
			...
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/community/my/records`,
				{
...
<div class="createPage">
  ...
  <div class="createContainer">
    ...
    <div class="createForm">
        ...
        >
          ...
          <Select.Content>
            <Select.Item value="discussion">{
              $_('community.type.discussion')
            }</Select.Item>
            <Select.Item value="media">{
              $_('community.type.media')
            }</Select.Item>
            <Select.Item value="guide">{
              $_('community.type.guide')
            }</Select.Item>
            <Select.Item value="collab">{
              $_('community.type.collab')
            }</Select.Item>
            <Select.Item value="review">{
              $_('community.type.review')
            }</Select.Item>
      ...
      {#if newPost.type === 'collab'}
        <div class="formField">
          <span class="fieldLabel">
            ...
            {$_('community.participants.label')} <span class="text-red-500"
            >*</span>
          </span>
          <Input
            type="number"
      ...
      {#if newPost.type === 'review'}
        ...
        <div class="formField">
          <span class="fieldLabel">
            ...
            {$_('community.review.select_level')} <span class="text-red-500"
            >*</span>
          </span>
          {#if selectedLevel}
          ...
          {:else}
            ...
            <div class="attachmentList">
              ...
              {:else if reviewLevels.length === 0}
                <p class="attachmentEmpty">
                  {$_('community.review.no_eligible_levels')}
                </p>
              {:else}
                {#each reviewLevels as level}
                  <button
                    class="attachmentOption"
                    on:click={() => selectReviewLevel(level)}
                  >
        ...
        <div class="formField">
          <span class="fieldLabel">{
            $_('community.review.recommendation')
          }</span>
      ...
      <!-- Image Upload -->
      <div class="formField">
        <span class="fieldLabel">{$_('community.create.image')} ({
