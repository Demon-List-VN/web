<script lang="ts">
	...
	$: completedLevelsCount =
		mapPack?.mapPackLevels?.filter(
			(level: any) => (mapPackLevelProgress[level?.levelID] ?? 0) >= 100
		).length || 0;
	...
	async function fetchMapPackData() {
		...
		try {
			const res = await sdk.battlepassApi.mappacks.byId(mapPackWrapper.id).request({
				headers: {
...
<div class="mx-auto mt-10 max-w-6xl">
	{#if mapPackWrapper && mapPack}
		<div style={cssVars} class="space-y-8">
			<!-- Hero Header Card -->
			<div
				class="relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-background via-background to-muted/20 shadow-xl"
			>
				<div
					class="absolute inset-0 opacity-10"
					style="background: linear-gradient(135deg, {getDifficultyColor(
						mapPack?.difficulty
					)}40 0%, transparent 50%);"
				/>
				...
				<div class="relative p-8 md:p-10">
					<div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
						...
						<div class="flex flex-1 items-start gap-6">
							<div class="relative">
								<div
									class="absolute -inset-2 rounded-2xl bg-gradient-to-br opacity-20 blur-xl"
									style="background: linear-gradient(135deg, {getDifficultyColor(
										mapPack?.difficulty
									)}, {primaryColor});"
								/>
								<div
									class="relative flex h-20 w-20 items-center justify-center rounded-2xl border-2 shadow-lg"
									style="background: linear-gradient(135deg, {getDifficultyColor(
										mapPack?.difficulty
									)}30, {getDifficultyColor(
										mapPack?.difficulty
									)}10); border-color: {getDifficultyColor(mapPack?.difficulty)};"
								>
									<Map
										class="h-10 w-10"
							...
							<div class="flex-1 space-y-2">
								<div
									...
									style="background-color: {getDifficultyColor(
										mapPack?.difficulty
									)}20; color: {getDifficultyColor(mapPack?.difficulty)};"
								>
									{getDifficultyName(mapPack?.difficulty)}
								</div>
								<h1 class="text-3xl font-bold tracking-tight md:text-4xl">
									{mapPack?.name || 'Map Pack'}
								</h1>
						...
						<div class="flex flex-col items-end gap-3">
							<div class="relative">
								<div
									class="absolute -inset-2 animate-pulse rounded-2xl bg-gradient-to-r opacity-30 blur"
									style="background: linear-gradient(135deg, {primaryColor}, {getDifficultyColor(
										mapPack?.difficulty
									)});"
								/>
								<div
									class="relative rounded-2xl border-2 px-6 py-4 shadow-lg backdrop-blur"
									style="background: linear-gradient(135deg, rgba(var(--primary-color), 0.15), rgba(var(--primary-color), 0.05)); border-color: {primaryColor}40;"
								>
									<div class="text-center">
										<div class="text-3xl font-black" style="color: {primaryColor};">
											+{mapPack?.xp || 0}
										</div>
										<div class="text-xs font-medium uppercase tracking-wider opacity-60">
											XP Reward
										</div>
									...
									<div class="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-1.5">
										...
										<span class="text-muted-foreground"
											>{$_('battlepass.unlocks_week', {
												values: { week: mapPackWrapper.unlockWeek }
											})}</span
										>
									</div>
								{/if}
						...
						<div class="mt-8 space-y-3">
							<div class="flex items-center justify-between">
								...
								<span
									...
									style="color: {getDifficultyColor(mapPack?.difficulty)};"
									>{localMapPackWrapper?.progress?.progress ?? 0}%</span
								>
							</div>
							<div class="relative h-4 overflow-hidden rounded-full bg-muted/50 shadow-inner">
								<div
									class="h-full rounded-full shadow-lg transition-all duration-700 ease-out"
									style="width: {localMapPackWrapper?.progress?.progress ??
										0}%; background: linear-gradient(90deg, {getDifficultyColor(
										mapPack?.difficulty
									)}, {primaryColor});"
								/>
							</div>
							<div class="flex items-center justify-between text-xs text-muted-foreground">
								<span
									...
									{$_('battlepass.levels_in_pack')}</span
								>
			...
			<div class="space-y-4">
				...
				<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{#each mapPack?.mapPackLevels || [] as level, i}
						...
						<div
							class="relative overflow-hidden rounded-xl border bg-card shadow-sm transition-all duration-300 {levelCompleted
								? 'border-green-500/50 bg-gradient-to-br from-green-500/5 to-green-500/0'
								: ''}"
						>
							{#if levelCompleted}
								<div
									class="absolute right-0 top-0 h-20 w-20 -translate-y-8 translate-x-8 rounded-full bg-gradient-to-br from-green-500/30 to-green-500/0 blur-2xl"
								/>
							{/if}
							...
							<div class="relative p-5">
								<div class="mb-4 flex items-start justify-between">
									<div class="flex items-start gap-3">
										<div
											class="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-lg font-bold transition-transform duration-300 {levelCompleted
										...
										>
											{#if levelCompleted}
												<Check class="h-6 w-6" />
												<div
													class="absolute -inset-1 animate-pulse rounded-xl bg-green-500/30 blur"
												/>
											{:else}
												<span>{i + 1}</span>
											{/if}
										...
										<div class="min-w-0 flex-1">
											<h3 class="truncate font-semibold leading-tight">
												{level.levels?.name || `Level ${level.levelID}`}
											</h3>
									...
									<div class="flex items-center gap-1.5">
										{#if levelCompleted}
											<div
												class="rounded-full bg-green-500/20 px-2 py-1 text-xs font-semibold text-green-600"
											>
												+25 XP ✓
											</div>
										{:else}
											<div
												class="rounded-full bg-muted px-2 py-1 text-xs font-semibold text-muted-foreground"
											>
												+25 XP
											</div>
										{/if}
								...
								{#if $user.loggedIn}
									<div class="space-y-2">
										...
										<div class="h-2 overflow-hidden rounded-full bg-muted">
											<div
												class="h-full rounded-full bg-gradient-to-r from-blue-500 via-cyan-500 to-teal-500 shadow-lg transition-all duration-500"
