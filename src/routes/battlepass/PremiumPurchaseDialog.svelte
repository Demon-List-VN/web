...
<Dialog.Root bind:open={open}>
  <Dialog.Content class="max-w-md">
      ...
      <div class="flex flex-col gap-[5px]">
        <Button
          ...
          class={`h-[70px] justify-start ${nextState == 2 ? 'border-white' : ''} ${
              alreadyPremium ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          on:click={() => {
              if (alreadyPremium) {
    ...
    {:else if state == 1}
      ...
      <Dialog.Footer>
        <Button variant="outline" on:click={() => (state = 0)}>{
          $_('general.back')
        }</Button>
        <Button disabled={!giftTo} on:click={() => (state = 2)}>{
          $_('general.next')
        }</Button>
      </Dialog.Footer>
    {:else}
      ...
      <div class="mt-4 flex flex-col gap-4">
        ...
        <div class="rounded-lg border-2 p-4">
          <div class="mb-3 flex items-center justify-between">
            <span class="font-medium">{seasonTitle} {
                $_('battlepass.premium_pass')
              }</span>
