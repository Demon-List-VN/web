<script lang="ts">
	...
	async function fetchMissions() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/battlepass/season/${seasonId}/missions`
			);
	...
	async function fetchMissionStatus() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/battlepass/missions`,
				{
					headers: {
...
{:else}
  <div class="flex flex-col gap-4">
    {#each missions as mission}
      ...
      <Card.Root
        class="
          overflow-hidden border-2 transition-all {isCompleted && !isClaimed
      ...
      >
        <Card.Content class="flex items-center gap-4 p-4">
          <div
            class="
              flex h-14 w-14 items-center justify-center rounded-xl {isCompleted
              ? 'bg-green-500/20'
              : 'bg-muted'}
            "
          >
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <h4 class="font-bold {isClaimed ? 'line-through' : ''}">
                {mission.title}
              </h4>
              {#if mission.refreshType && mission.refreshType !== 'none'}
                <span
                  class="flex items-center gap-1 rounded px-1.5 py-0.5 text-xs font-medium {mission.refreshType === 'daily' ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}"
                >
