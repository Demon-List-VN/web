<script lang="ts">
	...
	$: filteredOrders = orders.filter((order) => {
		...
		if (deliveredFilter === 'yes' && !order.delivered) {
			return false;
		}

		if (deliveredFilter === 'no' && order.delivered) {
			return false;
		}
	...
	function getProductName(order: any): string {
		...
		if (
			order.recordCards?.length > 0
		) {
			return `Store Items (${order.recordCards.length} Thẻ Bản Ghi)`;
		}
	...
	async function restore(order: any) {
		...
		window.location.href =
			`${import.meta.env.VITE_API_URL}/payment/success?orderCode=${order.id}`;
	}
	...
	function restorable(order: any) {
		...
		return order.state == 'CANCELLED' || order.state == 'EXPIRED'
			|| order.delivered;
	}
    ...
    <div class="flex flex-col gap-3">
      ...
      {:else}
        {#each pagedOrders as order}
          <Card.Root
            class="overflow-hidden transition-shadow hover:shadow-md rounded-xl"
          >
            <!-- Header -->
            ...
            <Card.Content class="px-5 py-4">
              <p class="font-medium text-base mb-2 truncate">
                {getProductName(order)}
              </p>
              <div class="flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground">
                <span><span class="text-foreground/60">Số lượng:</span> {
                    order.quantity ?? '?'
                  }</span>
                <span>
