<script lang="ts">
	...
	function resetInstallDialogScroll() {
		if (!shouldResetDialogScroll) {
			return;
		}

		document.querySelector<HTMLElement>('.installDialog')
			?.scrollTo({ top: 0 });
	}
...
<Dialog.Root bind:open={installDialogOpen}>
  <Dialog.Content class="installDialog">
    <Dialog.Header>
      ...
      <Dialog.Description>{
        $_('geode_mods.popup_description')
      }</Dialog.Description>
    </Dialog.Header>
...
.installSection {
  ...
  background:
    linear-gradient(135deg, hsl(var(--primary) / 0.08), transparent 42%),
    hsl(var(--card));
  border-radius: 8px;
}
