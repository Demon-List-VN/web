<script lang="ts">
	...
	function readFiltersFromURL() {
		...
		selectedProducts = p ? new Set(p.split(',')
			.map(Number)) : new Set();
	...
	function syncFiltersToURL() {
		...
		if (deliveredFilter) {
			params.set('delivered', deliveredFilter);
		}
		...
		if (amountMin !== null && amountMin > 0) {
			params.set('amountMin', String(amountMin));
		}

		if (amountMax !== null && amountMax > 0) {
			params.set('amountMax', String(amountMax));
		}
	...
	$: pagedOrders = filteredOrders.slice(
	...
	$: filteredOrders = allOrders.filter((o) => {
		if (selectedStates.size > 0 && !selectedStates.has(o.state)) {
			return false;
		}

		if (
			selectedMethods.size > 0 && !selectedMethods.has(o.paymentMethod)
		) {
			return false;
		}
		...
		if (selectedProducts.size > 0) {
			...
			if (!pids.some((id) => selectedProducts.has(id))) {
				return false;
			}
		}

		if (deliveredFilter === 'yes' && !o.delivered) {
			return false;
		}

		if (deliveredFilter === 'no' && o.delivered) {
			return false;
		}

		const total = o.amount + o.fee;

		if (amountMin !== null && amountMin > 0 && total < amountMin) {
			return false;
		}

		if (amountMax !== null && amountMax > 0 && total > amountMax) {
			return false;
		}
	...
	async function fetchOrders() {
		...
		try {
			...
			const url =
				`${import.meta.env.VITE_API_URL}/merchant/orders/all?${params.toString()}`;
	...
	async function addTracking() {
		if (!selectedOrder || !trackingContent) {
			return;
		}
...
<div class="filters">
        ...
        <div class="multi-select-list">
          {#each products as p}
            ...
            >
              <span
                class="check-icon"
                class:visible={selectedProducts.has(p.id)}
              >
  ...
  <div class="filter-group">
    ...
    <Select.Root
      ...
      selected={deliveredFilter
      ? {
          value: deliveredFilter,
          label: deliveredFilter === 'yes' ? 'Yes' : 'No'
      }
      : { value: 'all', label: 'All' }}
    >
  ...
  <Table.Caption>
    ...
    {:else}
      Showing {currentPage * PAGE_SIZE + 1}–{
        Math.min((currentPage + 1) * PAGE_SIZE, filteredOrders.length)
        ...
        <Table.Cell>
          <div class="action-buttons">
            <Button
              size="sm"
              variant="outline"
              on:click={() => openStateDialog(order)}
            >
              State
            </Button>
            <Button
              size="sm"
              variant="outline"
              on:click={() => openTrackingDialog(order)}
            >
              Track
            </Button>
          </div>
...
<Dialog.Root bind:open={showStateDialog}>
  <Dialog.Content class="max-w-sm">
    ...
    <Dialog.Footer>
      <Button variant="outline" on:click={() => (showStateDialog = false)}
      >Cancel</Button>
...
<Dialog.Root bind:open={showTrackingDialog}>
  <Dialog.Content class="max-w-sm">
    ...
    <Dialog.Footer>
      <Button variant="outline" on:click={() => (showTrackingDialog = false)}
      >Cancel</Button>
