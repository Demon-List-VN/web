<script lang="ts">
	...
	function getPublicListItemValue(stat: RecordPublicListStat) {
		if (
			stat.item?.position != null
			&& (stat.list.topEnabled || stat.list.mode === 'top')
		) {
		...
		toast.promise(
			fetch(
				`${import.meta.env.VITE_API_URL}/records/${record.data.userid}/${record.data.levelid}${
					record.data.id ? `?id=${record.data.id}` : ''
				}`,
				{
					method: 'DELETE',
					headers: { Authorization: 'Bearer ' + (await $user.token())! }
	...
	async function resetDeathCount() {
		...
		toast.promise(
			(async () => {
				...
				if (!response.ok) {
					throw new Error(
						(await response.text()) || 'Failed to reset death count'
					);
				}
		...
		toast.promise(
			(async () => {
				const res = await fetch(
					`${import.meta.env.VITE_API_URL}/inventory/item/15/consume`,
					{
						method: 'DELETE',
						headers: {
							Authorization: 'Bearer ' + (await $user.token()),
	...
	async function purchaseQueueBoost() {
		...
		const res: any = await (
			await fetch(
				`${import.meta.env.VITE_API_URL}/payment/getPaymentLink/5/${
					daysToSkip[0]
				}`,
				{
					method: 'POST',
	...
	$: totalBoosts = queueBoostInventory.reduce(
		(s, i) => s + (i.inventoryQuantity || 1),
		0
	);
...
<svelte:head>
  {#if record?.data}
    <title>
      {record.data.players.name} – {record.data.levels.name} · {
        $_('head.site_short_name')
      }
    </title>
  {/if}
</svelte:head>
  ...
  <div class="content-root">
    ...
    {:else}
      ...
      {#if showHero}
        <div
          class="hero-section"
          in:fly={{ y: 40, duration: 700, easing: quintOut }}
        >
          <div class="hero-inner">
            ...
            <div
              class="hero-stat"
              in:scale={{ duration: 700, delay: 250, easing: elasticOut, start: 0.7 }}
            >
            ...
            <div
              class="hero-level"
              in:fly={{ y: 20, duration: 600, delay: 350, easing: quintOut }}
            >
              <div class="hero-level-name">{record.data.levels.name}</div>
              <div class="hero-level-meta">
                {#if record.data.levels.creator}by {
                    record.data.levels.creator
                  }{/if}
              </div>
              ...
              {#if record.data.dlPt || record.data.flPt || record.data.plPt || record.data.clPt}
                <div class="points-row">
                  {#if record.data.dlPt}<span class="pt-badge dl"><Trophy
                        size={11}
                      />{record.data.dlPt} DL</span>{/if}
                  {#if record.data.flPt}<span class="pt-badge fl"><Star
                        size={11}
                      />{record.data.flPt} FL</span>{/if}
                  {#if record.data.plPt}<span class="pt-badge pl"><Zap
                        size={11}
                      />{record.data.plPt} PL</span>{/if}
                  {#if record.data.clPt}<span class="pt-badge cl"><Shield
                        size={11}
                      />{record.data.clPt} CL</span>{/if}
                </div>
              {/if}
            </div>
          </div>
          ...
          <Tabs.Root bind:value={activeTab}>
            <div class="tabs-bar">
              <div class="custom-tabs" role="tablist">
                ...
                <button
                  class="tab-btn"
                  class:active={activeTab === 'deathCount'}
                  on:click={() => activeTab = 'deathCount'}
                  role="tab"
                >
                  <Activity size={14} />
                  <span>{$_('record_detail.tabs.death_count')}</span>
                  {#if activeTab === 'deathCount'}<div class="tab-glow">
                    </div>{/if}
                </button>
                <button
                  class="tab-btn"
    ...
    && $user.data.uid === record.data.userid}
                  <button
                    class="tab-btn"
                    class:active={activeTab === 'skipAhead'}
                    on:click={() => activeTab = 'skipAhead'}
                    role="tab"
                  >
                    <SkipForward size={14} />
                    <span>{$_('record_detail.tabs.skip_ahead')}</span>
                    {#if activeTab === 'skipAhead'}<div class="tab-glow">
                      </div>{/if}
                  </button>
                {/if}
    ...
    && record.data.reviewer.uid === $user.data.uid && !record.data.needMod}
                  ...
                  >
                    ...
                    {#if activeTab === 'review'}<div class="tab-glow">
                      </div>{/if}
                  </button>
                {/if}
                ...
                <div class="glass-card">
                  ...
                  <div class="card-rows">
                    <div class="info-row">
                      ...
                      <a
                        href={record.data.videoLink}
                        target="_blank"
                        class="row-link"
                      >
                        {record.data.videoLink.slice(0, 45)}{
                          record.data.videoLink.length > 45 ? '…' : ''
                        }
                      </a>
                    </div>
    ...
    && record.data.raw}
                      <div class="info-row">
                        ...
                        <a
                          href={record.data.raw}
                          target="_blank"
                          class="row-link"
                        >
                          {record.data.raw.slice(0, 45)}{
                            record.data.raw.length > 45 ? '…' : ''
                          }
                        </a>
                      </div>
                    {/if}
...
.toLocaleString('vi-VN')
                    ...
                    {#if $user.loggedIn && $user.data.uid === record.data.userid}
                      <div class="info-row">
                        <span class="row-label">{
                          $_('record_detail.prioritized_by')
                        }</span>
                        <span class="row-badge">
                          {
                            Math.floor(record.data.prioritizedBy / 86400000)
                          }&nbsp;{$_('general.day')}{
                            Math.floor(record.data.prioritizedBy / 86400000) > 1 && $locale === 'en'
                            ? 's'
                            : ''
                          }
                        </span>
                ...
                <div class="glass-card">
                  ...
                  <div class="card-rows">
                    <div class="info-row">
                      <span class="row-label">{
                        $_('record_detail.device')
                      }</span>
                      <span class="row-value">
                        <span
                          class="device-badge {record.data.mobile ? 'mobile' : 'pc'}"
                        >
                          {#if record.data.mobile}<Smartphone
                              size={11}
                            />{:else}<Monitor size={11} />{/if}
                          {record.data.mobile ? 'Mobile' : 'PC'}
                        </span>
                        {#if record.data.refreshRate}
                          <span class="fps-badge">{
                              record.data.refreshRate
                            }fps</span>
                        {/if}
                      </span>
                    </div>
                    <div class="info-row">
                      <span class="row-label">{
                        $_('record_detail.rating')
                      }</span>
                      <span class="row-value row-highlight">
                        {record.data.suggestedRating ?? 'N/a'}
                        {#if record.data.progress === 100 && $user.loggedIn
    && $user.data.uid === record.data.players.uid}
                          <button
                            class="edit-inline-btn"
                            on:click={() => (open1 = !open1)}
                          >
                            <Pencil size={13} />
                          </button>
                        {/if}
                ...
                <div class="glass-card">
                  ...
                  <div class="card-rows">
                    ...
                    <div class="info-row">
                      ...
                      <span class="row-value">
                        {#if record.data.reviewedByOverwatch}
                          <span class="mod-badge">Overwatch consensus</span>
                        {:else if !record.data.isChecked && !record.data.reviewer}
                    ...
                    <div class="info-row">
                      <span class="row-label">{
                        $_('record_detail.need_mod_inspection')
                      }</span>
                      <span class="row-value">
                        <span
                          class="status-dot {record.data.needMod ? 'warn' : 'ok'}"
                        ></span>
                        {
                ...
                {#if publicListStats.length > 0}
                  <div class="glass-card wide">
                    <div class="card-header">
                      ...
                      <span>{
                        t('Thống kê list công khai', 'Public list stats')
                      }</span>
                    </div>
                    <div class="public-list-stat-grid">
                      {#each publicListStats as stat}
                        <a
                          class="public-list-stat-card"
                          href={getPublicListHref(stat)}
                        >
                          <div class="public-list-copy">
                            ...
                            <span class="public-list-meta">{
                              getPublicListMeta(stat)
                            }</span>
                          </div>
                  ...
                  <div class="flex flex-col gap-4">
                    ...
                    <div class="flex items-center justify-between">
                      <p class="font-semibold">
                        {$_('record_detail.skip_ahead.total')}
                      </p>
                      <p class="text-lg font-bold neon-text">
                        {formatPrice(5000 * daysToSkip[0])}₫
                      </p>
                    </div>
                    <Button
                ...
                {:else}
                  <div class="flex flex-col gap-4">
                    <h3 class="text-lg font-semibold">
                      {$_('payment.review.title')}
                    </h3>
                    <div class="flex text-sm">
                      <p>
                        {$_('record_detail.skip_ahead.queue_boost')} ({
                          daysToSkip[0]
                        } {
                          daysToSkip[0] > 1 && $locale === 'en'
                          ? $_('record_detail.skip_ahead.days')
                    ...
                    <div class="flex gap-2">
                      <Button
                        variant="outline"
                        on:click={() => (skipAheadState = 0)}
                      >{$_('general.back')}</Button>
                      {#if totalBoosts > 0 && totalBoosts >= daysToSkip[0]}
                        <Button
                          class="flex-1"
                          variant="secondary"
                          on:click={consumeQueueBoost}
                        >{$_('record_detail.skip_ahead.use_inventory')}</Button>
                        <Button class="flex-1" on:click={purchaseQueueBoost}>{
                          $_('record_detail.skip_ahead.purchase')
                        }</Button>
                      {:else}
                        <Button class="flex-1" on:click={purchaseQueueBoost}>{
              ...
              <div class="glass-card">
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <Label for="opt-two"
                    >Need further inspection (will be forwarded to a
                      moderator).</Label>
                  </div>
                  <Textarea
                    bind:value={cmt}
