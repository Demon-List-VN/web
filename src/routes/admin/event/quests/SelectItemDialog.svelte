<script lang="ts">
	...
	async function addReward() {
		...
		toast.promise(
			fetch(
				`${import.meta.env.VITE_API_URL}/events/${event.id}/quest/${quest.id}/reward`,
				{
					method: 'POST',
					headers: {
						Authorization: 'Bearer ' + (await $user.token()),
...
<Dialog.Root bind:open>
  ...
  <Dialog.Content class="sm:max-w-[500px]">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Search and select an item to add as reward for quest: {
    ...
    <div class="grid gap-4 py-4">
      {#if selectedItem}
        ...
        <p class="text-xs text-muted-foreground">
          Leave empty for permanent reward. Enter number of days until the
          reward expires.
        </p>
      {:else}
        <div class="relative">
