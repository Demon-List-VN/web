<script lang="ts">
	...
	async function fetchQuests() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/events/${event.id}/quest`,
				{
					method: 'GET',
					headers: {
	...
	async function deleteQuest(questId: number) {
		...
		toast.promise(
			fetch(
				`${import.meta.env.VITE_API_URL}/events/${event.id}/quest/${questId}`,
				{
					method: 'DELETE',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())
	...
	async function removeReward(questId: number, rewardId: number) {
		if (!confirm('Remove this reward from the quest?')) {
			return;
		}

		toast.promise(
			fetch(
				`${import.meta.env.VITE_API_URL}/events/${event.id}/quest/${questId}/reward/${rewardId}`,
				{
					method: 'DELETE',
					headers: {
						Authorization: 'Bearer ' + (await $user.token())
