<script lang="ts">
	...
	async function removeCourse(id: number) {
		...
		toast.promise(
			(async () => {
				const res = await fetch(
					`${import.meta.env.VITE_API_URL}/battlepass/course/${id}`,
					{
						method: 'DELETE',
						headers: { Authorization: `Bearer ${await $user.token()}` }
					}
	...
	async function removeCourseEntry(id: number) {
		...
		toast.promise(
			(async () => {
				const res = await fetch(
					`${import.meta.env.VITE_API_URL}/battlepass/course/entry/${id}`,
					{
						method: 'DELETE',
						headers: { Authorization: `Bearer ${await $user.token()}` }
					}
        ...
        {#each courses as course}
          <div
            class={`rounded border p-3 ${
                selectedCourseId === course.id ? 'border-primary bg-muted/40' : ''
            }`}
          >
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="flex shrink-0 gap-2">
                <Button
                  variant="outline"
                  size="icon"
                  on:click={() => openEditCourse(course)}
                >
                  <Edit class="h-4 w-4" />
                </Button>
                <Button
