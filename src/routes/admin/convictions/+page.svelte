	...
	async function fetchConvictions(uid: string) {
		...
		try {
			const response = await fetch(
				`${import.meta.env.VITE_API_URL}/players/${uid}/convictions`,
				{
			...
			toast.error(
				error?.message || $_('admin_convictions.toast.fetch_error')
			);
		} finally {
			loadingList = false;
		}
...
<div class="wrapper">
  ...
  {#if selectedPlayer?.uid}
    <div class="mt-8">
      ...
      {:else}
        <Table.Root>
          <Table.Caption>{$_('admin_convictions.total')}: {
              convictions.length
            }</Table.Caption>
          <Table.Header>
            <Table.Row>
              ...
              <Table.Head class="w-[120px] text-center">{
                $_('admin_convictions.credit_reduce')
              }</Table.Head>
              <Table.Head class="w-[220px] text-center">{
                $_('admin_convictions.created_at')
              }</Table.Head>
