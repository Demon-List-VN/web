<script lang="ts">
	...
	let deathCountArray: number[] = Array(100)
		.fill(0);
	...
	async function simulateDeathCount() {
		try {
			...
			const url =
				`${import.meta.env.VITE_API_URL}/deathCount/${deathCountData.levelID}/${countString}${
					deathCountData.completed ? '?completed=true' : ''
				}`;
	...
	async function simulateEventSubmit() {
		try {
			if (
				!eventSubmitData.levelID || !eventSubmitData.progress
				|| !eventSubmitData.password
			) {
			...
			const url =
				`${import.meta.env.VITE_API_URL}/events/submitLevel/${eventSubmitData.levelID}?progress=${eventSubmitData.progress}&password=${
					encodeURIComponent(eventSubmitData.password)
				}`;
	...
	async function copyDeathCountAsCurl() {
		try {
			...
			const cmd = await buildCurlCommand('POST', url, {
	...
	async function copyEventSubmitAsCurl() {
		try {
			...
			const token = await $user.token();
			const cmd = await buildCurlCommand('PUT', url, {
				Authorization: 'Bearer ' + token
			});
			await copyToClipboard(cmd);
			toast.success('Copied cURL to clipboard');
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-4">
          ...
          <div class="form-group">
            ...
            <p class="helper-text">
              Log death attempts at specific checkpoints. The count will be
                ...
                {#each attempts as attempt, index}
                  <div class="attempt-item">
                    <span class="attempt-info">
                      <strong>{attempt.percent}%</strong> → +{attempt.count}
                      deaths
                    </span>
                    <Button
                      variant="destructive"
      ...
      <Card.Root>
        ...
        <Card.Footer class="flex justify-between">
          ...
          <div class="flex items-center space-x-2">
            <Button
              variant="outline"
              on:click={copyEventSubmitAsCurl}
              disabled={eventSubmitLoading}
            >
              Copy as cURL
            </Button>
            <Button
              on:click={simulateEventSubmit}
              disabled={eventSubmitLoading}
            >
