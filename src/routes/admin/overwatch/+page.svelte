<script lang="ts">
	...
	import { Activity, AlertTriangle, ArrowRight, BarChart3, Check, CheckCircle2, Clock3, Gauge, Minus, RefreshCw, Search, ShieldCheck, Users, Workflow, X, Zap } from 'lucide-svelte';
	  ...
	  {:else if metrics}
		<section class="case-workspace">
		  ...
		  <section class="panel case-table-card">
			<div class="panel-header"><div><span class="section-kicker">ACTIVE POOL</span><h2>Records under review</h2><p>All {metrics.poolSize} pool slots appear here with the current status of each case.</p></div><Badge variant="outline">{metrics.poolRecords.length} / {metrics.poolSize}</Badge></div>
			<div class="table-scroll"><Table.Root><Table.Header><Table.Row><Table.Head>Record</Table.Head><Table.Head>Player</Table.Head><Table.Head>Status</Table.Head><Table.Head class="text-center">Votes</Table.Head><Table.Head>Phase</Table.Head><Table.Head class="text-center">Round</Table.Head><Table.Head class="text-center">Reviewers</Table.Head><Table.Head>In pool</Table.Head></Table.Row></Table.Header><Table.Body>
			  {#each metrics.poolRecords as item (item.caseId)}
				<Table.Row><Table.Cell><div class="record-cell">{#if recordHref(item)}<a href={recordHref(item) ?? '#'}>{item.levelName}</a>{:else}<strong>{item.levelName}</strong>{/if}<small>#{item.recordId ?? '—'} · {number(item.progress)}%</small></div></Table.Cell><Table.Cell>{item.playerName}</Table.Cell><Table.Cell><span class="status-pill {statusTone(item.status)}">{statusLabel(item.status)}</span></Table.Cell><Table.Cell><div class="vote-status" aria-label={`${item.voteSummary.accept} accept votes, ${item.voteSummary.reject} reject votes, ${item.voteSummary.unsure} unsure votes`}><span class="vote-count accept" title="Accept votes"><Check size={13} strokeWidth={3} /><b>{item.voteSummary.accept}</b></span><span class="vote-count reject" title="Reject votes"><X size={13} strokeWidth={3} /><b>{item.voteSummary.reject}</b></span><span class="vote-count unsure" title="Unsure votes"><Minus size={13} strokeWidth={3} /><b>{item.voteSummary.unsure}</b></span></div></Table.Cell><Table.Cell><span class="phase-label">{statusLabel(item.phase)}</span></Table.Cell><Table.Cell class="text-center">{item.round}</Table.Cell><Table.Cell class="text-center">{item.requiredReviewers}</Table.Cell><Table.Cell>{duration(Date.now() - new Date(item.poolEnteredAt ?? item.submittedAt).getTime())}</Table.Cell></Table.Row>
			  {:else}<Table.Row><Table.Cell colspan={8} class="text-center text-muted-foreground">The review pool is empty.</Table.Cell></Table.Row>{/each}
			</Table.Body></Table.Root></div>
		  </section>

		  <div class="case-columns">
			<section class="panel case-table-card">
			  <div class="panel-header"><div><span class="section-kicker">UP NEXT</span><h2>Top 5 after pool</h2><p>Queue positions begin after the virtual {metrics.poolSize}-record pool.</p></div><Clock3 size={19} class="muted-icon" /></div>
			...
			<section class="panel case-table-card">
			  <div class="panel-header"><div><span class="section-kicker">RECENTLY CONCLUDED</span><h2>Latest verdicts</h2><p>Ten most recent cases with final verdicts.</p></div><CheckCircle2 size={19} class="muted-icon" /></div>
...
<style>
  ...
  .vote-status{display:flex;align-items:center;justify-content:center;gap:7px;min-width:102px}.vote-count{display:inline-flex;align-items:center;gap:2px;font-size:.7rem}.vote-count.accept{color:var(--ow-green)}.vote-count.reject{color:var(--ow-red)}.vote-count.unsure{color:hsl(var(--muted-foreground))}.vote-count b{font-variant-numeric:tabular-nums}
