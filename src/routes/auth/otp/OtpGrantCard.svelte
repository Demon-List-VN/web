<script lang="ts">
	...
	$: canGrant = normalizedCode.length === 6 && $user.checked && $user.loggedIn && !loading;
	...
	function getGrantErrorMessage(message: unknown) {
		if (typeof message !== 'string') return $_('otp.error');
	...
	async function grant() {
		...
		try {
			const res = await fetch(`${import.meta.env.VITE_API_URL}/auth/otp/${normalizedCode}`, {
				method: 'PATCH',
				headers: {
			...
			if (res.ok) {
				...
				const targetPath = `${getLocalePrefix()}/auth/otp/${normalizedCode}`;
	...
	<Card.Root class="w-full max-w-md">
		{#if granted}
			<Card.Header class="items-center text-center">
				...
				<Card.Title class="text-2xl text-green-500">{$_('otp.success')}</Card.Title>
		...
		{:else}
			...
			<Card.Content class="flex flex-col items-center gap-4">
				<div class="w-full space-y-2">
					<label class="text-sm font-medium" for="otp-code">{$_('otp.code_label')}</label>
					<Input
						id="otp-code"
				...
				{#if !$user.checked}
					<p class="py-2 text-sm text-muted-foreground">{$_('general.loading')}...</p>
				{:else if !$user.loggedIn}
					<p class="py-2 text-sm text-muted-foreground">{$_('otp.not_logged_in')}</p>
				{:else}
					{#if error}
