<script lang="ts">
	...
	async function createList() {
		...
		try {
			...
			if (!res.ok) {
				throw new Error(
					payload.error || $_('custom_lists.toast.failed_create')
				);
			}
			...
			toast.error(
				error instanceof Error
					? error.message
					: $_('custom_lists.toast.failed_create')
			);
		} finally {
			creating = false;
		}
...
<div class="page">
  ...
  {:else}
    <div class="formCard">
      <div class="formGrid">
        <div class="field">
          <label for="list-title">{$_('custom_lists.new.title_label')} <span
              class="required"
            >*</span></label>
          <Input
            id="list-title"
            ...
            maxlength={100}
            placeholder={$_('custom_lists.new.title_placeholder')}
          />
        </div>

        <div class="field">
          <label for="list-description">{
            $_('custom_lists.new.description_label')
          }</label>
          <Textarea
            id="list-description"
        ...
        <div class="field">
          <span class="fieldLabel">Visibility <span class="required"
            >*</span></span>
          <div class="optionRow">
            <button
              type="button"
        ...
        <div class="field">
          <span class="fieldLabel">List type <span class="required"
            >*</span></span>
          <div class="optionRow">
            <button
              type="button"
      ...
      <div class="actionRow">
        <Button on:click={createList} disabled={creating}>
          ...
          {
            creating
            ? $_('custom_lists.new.creating')
            : $_('custom_lists.new.create_button')
          }
        </Button>
        <Button
          variant="outline"
