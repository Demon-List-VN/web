<script lang="ts">
	...
	async function loadInvitation(force: boolean = false) {
		...
		try {
			...
			if (!res.ok || !payload) {
				throw new Error(
					payload?.error
					|| $_('custom_lists.collaborator_invitation.load_failed')
				);
			}
			...
			loadingError = error instanceof Error
				? error.message
				: $_('custom_lists.collaborator_invitation.load_failed');
		} finally {
			loading = false;
	...
	async function acceptPendingInvitation() {
		if (!list || !canRespondToInvitation) {
			return;
		}
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/lists/${list.id}/invitations/accept`,
				{
					method: 'POST',
					headers: {
						Authorization: `Bearer ${await $user.token()}`
			...
			if (!res.ok || !payload) {
				throw new Error(
					payload?.error
					|| $_('custom_lists.toast.failed_accept_invitation')
				);
			}
	...
	async function rejectPendingInvitation() {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API_URL}/lists/${list.id}/invitations/reject`,
				{
					method: 'POST',
					headers: {
						Authorization: `Bearer ${await $user.token()}`
			...
			if (!res.ok) {
				throw new Error(
					payload?.error
					|| $_('custom_lists.toast.failed_reject_invitation')
				);
			}
	...
	$: isCollaborator = Boolean(
		list && list.currentUserRole && list.currentUserRole !== 'viewer'
	);
	$: if ($user.checked && $user.loggedIn) {
		void loadInvitation();
	}
...
<svelte:head>
  <title>
    {$_('custom_lists.collaborator_invitation.title')} - {$_('head.site_name')}
  </title>
</svelte:head>
...
<div class="page">
  <header class="topBar">
    <div class="topBarLeft">
      ...
      <span class="topBarLabel">{
        $_('custom_lists.collaborator_invitation.title')
      }</span>
    </div>
  </header>
  ...
  {:else if list && isCollaborator}
    <section class="stateCard successState">
        ...
        <h2>
          {
            $_(
                justAccepted
                    ? 'custom_lists.collaborator_invitation.accepted_title'
                    : 'custom_lists.collaborator_invitation.already_member_title'
            )
          }
        </h2>
        <p>
          {
            $_(
                justAccepted
                    ? 'custom_lists.collaborator_invitation.accepted_hint'
                    : 'custom_lists.collaborator_invitation.already_member_hint'
            )
          }
        </p>
      </div>
      <div class="stateActions">
        <Button
          on:click={() => goto(`/lists/${$page.params.id}/manage?tab=collaboration`)}
        >
