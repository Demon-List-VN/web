<script lang="ts">
	...
	export let setBanState: (
		nextIsBanned: boolean,
		confirmationName: string
	) => void | Promise<void> = async () => {};
	export let deleteList: (confirmationName: string) => void | Promise<void> =
		async () => {};
	...
	$: hasDangerConfirmation = Boolean(
		list && dangerConfirmName.trim() === list.title
	);
</script>
...
<div class="tabContent">
  <div class="toolCard dangerCard">
    ...
    <div class="field">
      <label for="danger-confirm-name">{
        $_('custom_lists.manage.confirm_title_label')
      }</label>
      <Input
      ...
      <p class="hint">
        {
          $_('custom_lists.manage.confirm_title_hint', {
    ...
    <div class="formActions">
      ...
      {#if canDelete}
        <Button
          variant="destructive"
          on:click={() => deleteList(dangerConfirmName)}
          disabled={!hasDangerConfirmation}
        >
