<script lang="ts">
	...
	function updateRankBadge(index: number, patch: Record<string, any>) {
		editForm = {
			...editForm,
			rankBadges: editForm.rankBadges.map((
				rankBadge: any,
				currentIndex: number
	...
	function updateTextRankBadge(
		index: number,
		field: 'name' | 'shorthand' | 'color',
		event: Event
	) {
		updateRankBadge(index, { [field]: getInputValue(event) });
	}

	function updateNumberRankBadge(
		index: number,
		field: 'minRating' | 'minTop',
		event: Event
	) {
	...
	function removeRankBadge(index: number) {
		editForm = {
			...editForm,
			rankBadges: editForm.rankBadges.filter((_: any, currentIndex: number) =>
				currentIndex !== index
			)
		};
	}
	...
	function onRankBadgeDrop(event: DragEvent, targetIndex: number) {
		...
		if (
			draggedRankBadgeIndex === null || draggedRankBadgeIndex === targetIndex
		) {
			onRankBadgeDragEnd();
...
<div class="tabContent">
  <div class="toolCard">
    <h2 class="toolHeading">
      {$_('custom_lists.detail.edit.rank_badges_label')}
    </h2>
    <div class="formGrid">
      <div class="field">
        <div class="rankBadgeHeader">
          <div>
            ...
            <p class="hint">
              {$_('custom_lists.detail.edit.rank_badges_hint')}
            </p>
          </div>
          <Button
            type="button"
        ...
        {:else}
          <div class="rankBadgeList" role="list">
            {#each editForm.rankBadges as rankBadge, index}
              ...
              >
                <div
                  class="rankBadgeEditorHandle"
                  title={$_('custom_lists.detail.edit.rank_badges_priority_hint')}
                >
                  <GripVertical class="h-5 w-5" />
                </div>
                <div
                  class="rankBadgePreview"
                  style={`background: ${rankBadge.color || '#64748b'}`}
                >
                  {getRankBadgePreviewLabel(rankBadge)}
                </div>
                <div class="rankBadgeEditorFields">
                  <div class="field compactField">
                    <label for={`rank-badge-name-${index}`}>{
                      $_('custom_lists.detail.edit.rank_badges_name_label')
                    }</label>
                    <Input
                  ...
                  <div class="field compactField">
                    <label for={`rank-badge-shorthand-${index}`}>{
                      $_('custom_lists.detail.edit.rank_badges_shorthand_label')
                    }</label>
                    <Input
                  ...
                  <div class="field compactField">
                    <label for={`rank-badge-color-${index}`}>{
                      $_('custom_lists.detail.edit.rank_badges_color_label')
                    }</label>
                    <Input
                  ...
                  <div class="field compactField">
                    <label for={`rank-badge-min-rating-${index}`}>{
                      $_('custom_lists.detail.edit.rank_badges_min_rating_label')
                    }</label>
                    <Input
                  ...
                  <div class="field compactField">
                    <label for={`rank-badge-min-top-${index}`}>{
                      $_('custom_lists.detail.edit.rank_badges_min_top_label')
                    }</label>
                    <Input
