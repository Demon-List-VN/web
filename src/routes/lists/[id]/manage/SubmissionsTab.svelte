<script lang="ts">
	...
	export let rejectSubmission: (submission: any) => void | Promise<void> =
		async () => {};
	...
	function getLevelName(submission: any) {
		return submission?.level?.name?.trim()?.length
			? submission.level.name
			: `Level #${submission.levelId}`;
	}

	function getLevelCreator(submission: any) {
		return submission?.level?.creator?.trim()?.length
			? submission.level.creator
			: 'Unknown';
	}

	function getSubmitterName(submission: any) {
		return submission?.submitterData?.name?.trim()?.length
			? submission.submitterData.name
			: submission.addedBy || 'Unknown';
	}

	function getSubmitterHref(submission: any) {
		return submission?.submitterData?.uid || submission.addedBy
			? `/player/${submission.submitterData?.uid || submission.addedBy}`
			: null;
	}
	...
	function openReviewDialog(submission: any) {
		...
		minProgressValue = submission.minProgress == null
			? ''
			: String(submission.minProgress);
	...
	async function acceptActiveSubmission() {
		...
		if (isTopList) {
			...
			payload.minProgress = minProgressValue.trim().length
				? Number.parseInt(minProgressValue, 10)
				: null;
		} else {
...
<div class="tabContent">
  {#if canReviewSubmissions}
    <div class="toolCard queueCard">
      <div class="queueHeader">
        <div>
          <h2 class="toolHeading">
            {$locale == 'vi' ? 'Hàng đợi duyệt level' : 'Level review queue'}
          </h2>
          <p class="hint">
            {
              $locale == 'vi'
      ...
      {#if loading}
        <p class="hint">
          {$locale == 'vi' ? 'Đang tải hàng đợi...' : 'Loading queue...'}
        </p>
      ...
      {:else if submissions.length === 0}
        <p class="hint">
          {
            $locale == 'vi'
            ? 'Chưa có level nào chờ duyệt.'
            : 'No submitted levels are waiting for review.'
          }
        </p>
      {:else}
        <div class="queueList">
          {#each submissions as submission}
            <article class="queueItem">
              ...
              <div class="queueItemMain">
                ...
                <p class="creatorLine">
                  {$locale == 'vi' ? 'bởi' : 'by'} <strong>{
                    getLevelCreator(submission)
                  }</strong>
                </p>
...
<Dialog.Root bind:open={dialogOpen}>
  <Dialog.Content class="max-w-[620px]">
    <Dialog.Header>
      <Dialog.Title>{
        $locale == 'vi' ? 'Duyệt level đã nộp' : 'Review submitted level'
      }</Dialog.Title>
      <Dialog.Description>
        {
          $locale == 'vi'
    ...
    {#if activeSubmission}
      <div class="dialogBody">
        <div class="summaryCard">
          ...
          {#if activeSubmission.submissionComment}
            <p class="submissionComment">
              {activeSubmission.submissionComment}
            </p>
          {/if}
        </div>
        ...
        {:else}
          <div class="field">
            <Label for="submission-rating">{
              $locale == 'vi' ? 'Rating' : 'Rating'
            }</Label>
            <Input
        ...
        <div class="field">
          <Label for="submission-min-progress">
            {
              $locale == 'vi'
              ? (isPlatformer ? 'Base time' : 'Min progress')
              : (isPlatformer ? 'Base time' : 'Min progress')
            }
          </Label>
          <Input
    ...
    <Dialog.Footer class="gap-2">
      ...
      <Button
        variant="destructive"
        on:click={() => rejectActiveSubmission()}
        disabled={submittingReview}
      >
        <XCircle class="mr-2 h-4 w-4" />
        {$locale == 'vi' ? 'Từ chối' : 'Reject'}
