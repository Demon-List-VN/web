<script lang="ts">
	...
	async function submitSubmission() {
		...
		if (!videoLink) {
			toast.error(
				t(
					'Vui lòng nhập link video YouTube',
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					payload?.error
					|| t('Không thể gửi level', 'Failed to submit level')
				);
			}
	...
	function getHeroStyle(currentList: typeof list) {
		...
		const styles: string[] = [];
		const bg = isHexColor(currentList.backgroundColor)
			? String(currentList.backgroundColor)
				.trim()
			: null;
		const border = isHexColor(currentList.borderColor)
			? String(currentList.borderColor)
				.trim()
			: null;
		...
		if (bg) {
			...
			styles.push(
				`background: ${bg}; --custom-surface-foreground: ${
					light ? '#0f172a' : '#f8fafc'
				}; --custom-surface-muted: ${
					light ? 'rgba(15,23,42,0.72)' : 'rgba(248,250,252,0.78)'
				};`
			);
		}
...
<svelte:head>
  <title>
    {
      list
      ? `${list.title} | ${$_('head.titles.submit_level')}`
      : $_('head.titles.submit_level')
    }
  </title>
</svelte:head>
...
<div class="submit-page">
  {#if !$user.loggedIn}
    <div class="state-card">
      ...
      <h2>
        {
          $locale == 'vi' ? 'Đăng nhập để nộp level' : 'Sign in to submit a level'
        }
      </h2>
      <p class="state-hint">
        {
  ...
  {:else if !list}
    <div class="state-card">
      ...
      <h2>
        {$locale == 'vi' ? 'Không thể tải danh sách' : 'Unable to load list'}
      </h2>
      <p class="state-hint">
        {
          loadingError
  ...
  {:else if !list.levelSubmissionEnabled}
    <div class="state-card">
      ...
      <h2>
        {
          $locale == 'vi'
          ? 'Danh sách chưa mở nộp level'
          : 'Level submissions are closed'
        }
      </h2>
      <p class="state-hint">
        {
      ...
      <div class="hero-card" style={getHeroStyle(list)}>
        ...
        <div class="hero-body">
          ...
          <div class="hero-identity">
            ...
            <div>
              <p class="eyebrow">
                {$locale == 'vi' ? 'Nộp level' : 'Submit a level'}
              </p>
              <h1>{list.title}</h1>
            </div>
          </div>
        ...
        {#if submitted}
          <div class="success-state">
            ...
            <div class="success-text">
              <h2>
                {$locale == 'vi' ? 'Level đã được gửi!' : 'Level submitted!'}
              </h2>
              <p class="text-muted">
                {
                  $locale == 'vi'
                  ? `${
                      submissionResult?.levelName || 'Level'
                  } đã vào hàng đợi duyệt của ${list.title}.`
                  : `${
                      submissionResult?.levelName || 'The level'
                  } is now in the review queue for ${list.title}.`
                }
              </p>
            </div>
              ...
              <div class="success-details">
                {#if submissionResult?.videoID}
                  <div class="detail-row">
                    ...
                    <a
                      href={`https://youtu.be/${submissionResult.videoID}`}
                      target="_blank"
                      rel="noreferrer"
          ...
          <div class="step-footer">
            {#if step > 0}
              <Button
                variant="outline"
                on:click={back}
                disabled={levelLoading || submitting}
              >
