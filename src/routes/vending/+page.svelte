<script lang="ts">
	...
	const MATERIALS = [
		...
		},
		{
			...
			features: [
				'Chip NFC',
				'In mã QR',
				'Thẻ nhựa bóng cao cấp',
				'Kích thước chuẩn CR80'
			]
		}
	];

	$: bgImage = customImageDataUrl
		|| (selectedRecord
			? `https://levelthumbs.prevter.me/thumbnail/${selectedRecord.levelid}/high`
			: '');
	...
	async function fetchRecords() {
		...
		try {
			...
			records = (Array.isArray(data) ? data : []).filter((r: any) =>
				r.levelid
			);
            ...
            >
              ...
              <div class="record-info">
                ...
                <span class="record-pts">
                  {#if record.dlPt}DL: {
                      record.dlPt?.toFixed(0)
                    }pt{:else if record.flPt}FL: {
                      record.flPt?.toFixed(0)
                    }pt{:else if record.plPt}PL: {
                      record.plPt?.toFixed(0)
                    }pt{/if}
                </span>
              </div>
            </button>
          ...
          >
            {#if customImageDataUrl}
              <div class="upload-preview">
                ...
                <button
                  class="remove-img"
                  on:click={() => (customImageDataUrl = null)}
                  type="button"
                >
                  <X size={14} />
                </button>
            ...
            {:else}
              ...
              <p>
                Kéo thả hoặc <label class="upload-link">chọn ảnh<input
                    type="file"
                    accept="image/*"
              ...
              <p class="upload-hint">
                Mặc định: ảnh thumbnail YouTube của level
              </p>
            {/if}
          </div>
        </div>
          ...
          >
            {#if customAvatarDataUrl}
              <div class="upload-preview avatar-preview">
                ...
                <button
                  class="remove-img"
                  on:click={() => (customAvatarDataUrl = null)}
                  type="button"
                >
                  <X size={14} />
                </button>
            ...
            {:else}
              ...
              <p>
                Kéo thả hoặc <label class="upload-link">chọn ảnh<input
                    type="file"
                    accept="image/*"
                    on:change={onAvatarFileInput}
                    hidden
                  /></label>
              </p>
              <p class="upload-hint">
                Mặc định: ảnh đại diện tài khoản của bạn
              </p>
            {/if}
          </div>
        </div>
      ...
      {#if selectedMaterial}
        <div class="order-summary">
          <div class="summary-row">
            <span>Thẻ Bản Ghi ({
                selectedMaterial === 'paper' ? 'Giấy' : 'Nhựa'
              })</span>
            <span>{
              MATERIALS.find((m) => m.id === selectedMaterial)?.price
            }</span>
          </div>
