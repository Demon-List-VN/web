      ...
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
        <div class="space-y-3">
          <Label class="text-sm font-medium">{
            $_('event_filter.event_type')
          }</Label>
          <RadioGroup.Root bind:value={filter.eventType} class="space-y-1">
            <div class="flex items-center space-x-2">
              <RadioGroup.Item value="all" id="all-events" />
              <Label for="all-events" class="text-sm font-normal">{
                $_('event_filter.all_events')
              }</Label>
            </div>
            <div class="flex items-center space-x-2">
              <RadioGroup.Item value="contest" id="contest-only" />
              <Label for="contest-only" class="text-sm font-normal">{
                $_('event_filter.contest_only')
              }</Label>
            </div>
            <div class="flex items-center space-x-2">
              <RadioGroup.Item value="nonContest" id="non-contest" />
              <Label for="non-contest" class="text-sm font-normal">{
                $_('event_filter.non_contest')
              }</Label>
            </div>
          </RadioGroup.Root>
        </div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <Label
                ...
                class={`text-sm font-normal ${
                    filter.eventType !== 'contest' ? 'text-muted-foreground' : ''
                }`}
              >
