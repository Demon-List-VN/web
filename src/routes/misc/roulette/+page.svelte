 ...
 */
	function extractLevelFromRandomResponse(json: unknown): Level | null {
		...
		if (
			jsonObj.data && !Array.isArray(jsonObj.data)
		...
		if (
			jsonObj.data && Array.isArray(jsonObj.data) && jsonObj.data.length > 0
		) {
 ...
 */
	async function fetchMaxValues(): Promise<void> {
		...
		try {
			...
			maxDlTop = Number(
				(dlData as { levelCount?: number; } | null)?.levelCount ?? 1000
			);
			maxFlTop = Number(
				(flData as { levelCount?: number; } | null)?.levelCount ?? 1000
			);
 ...
 */
	function isLevelInRange(level: Level, type: 'dl' | 'fl' | 'pl'): boolean {
		...
		if (
			rangeStart !== ''
			&& (typeof topValue !== 'number' || topValue < Number(rangeStart))
		) {
			return false;
		}
		...
		if (
			rangeEnd !== ''
			&& (typeof topValue !== 'number' || topValue > Number(rangeEnd))
		) {
			return false;
		}
	...
	async function fetchRandomLevel(): Promise<void> {
		...
		try {
			while (attempts < MAX_RETRY_ATTEMPTS) {
				...
				const url =
					`${import.meta.env.VITE_API_URL}/lists/${type}/random?${CACHE_BUSTER_PARAM}=${Date.now()}`;
			...
			message =
				'No suitable levels found. Try adjusting your range or reset.';
			finished = true;
			currentLevel = null;
		} catch (error) {
 ...
 */
	function validateInput(value: string): boolean {
		...
		if (
			!/^[0-9]+$/.test(value) || isNaN(numericValue)
...
<div class="flex flex-col items-center min-h-screen dark:text-white text-black pb-10 px-2">
  ...
  <div class="dark:text-zinc-300 text-zinc-700 text-lg text-center mb-8">
    A roulette will select any Extreme Demon for you to try with increasing
    minimum percentages<br />
  ...
  <div class="flex flex-col md:flex-row gap-6 md:gap-10 mt-6 items-start w-full max-w-5xl">
    <Card
      class="rounded-2xl shadow-xl min-w-[0] w-full md:max-w-[520px] flex flex-col items-center dark:bg-black bg-white mb-6 md:mb-0"
    >
      <CardContent class="w-full flex-col items-center p-4 sm:p-8">
        ...
        {:else if currentLevel}
          <LevelCard
            {...toLevelCardProps(currentLevel, currentType)}
            type={currentType}
          />
        {:else if !finished}
          <div class="h-[120px] sm:h-[220px] flex items-center justify-center text-base sm:text-xl mb-4 text-zinc-500 dark:text-zinc-400">
        ...
        {#if !finished && currentLevel && targetPercent <= 100}
          <div class="flex flex-col items-center gap-2 mt-6 w-full">
            <Label
              for="percent"
              class="font-light text-base sm:text-lg dark:text-white text-black"
            >
    ...
    <Card
      class="rounded-2xl shadow-xl min-w-[0] w-full md:max-w-[350px] flex flex-col items-center dark:bg-black bg-white"
    >
      <CardContent
        class="w-full flex flex-col items-center p-4 sm:p-8 gap-3 sm:gap-5"
      >
        {#if isMaxValuesLoaded}
