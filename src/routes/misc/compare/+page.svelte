<script lang="ts">
	...
	function getBestRecord(records: any[], listType: 'dl' | 'pl') {
		...
		const filtered = records.filter((r: any) => {
			return r.levels?.dlTop && r.levels.dlTop > 0
				&& r.levels.isPlatformer == (listType == 'pl');
		});
        ...
        {#if player2}
          <div class="selected-player">
            ...
            <Button
              variant="ghost"
              size="icon"
              on:click={() => removePlayer(2)}
            >
              <X class="h-4 w-4" />
            </Button>
          </div>
        {:else}
          <Button class="w-full" on:click={() => openSelector(2)}
    ...
    <Card.Root class="comparison-card mt-8">
      <Card.Header>
        <Card.Title class="text-center">{
          $_('compare.statistics_comparison')
        }</Card.Title>
      </Card.Header>
      <Card.Content>
