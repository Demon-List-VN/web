<script lang="ts">
	...
	async function openCase() {
		...
		const targetPosition = winningDisplayIndex * (itemWidth + gap)
			+ itemWidth / 2;
		const finalPosition = targetPosition + Math.random() * itemWidth
			- itemWidth / 2;
		...
		await new Promise<void>((resolve) => {
			...
			function animate() {
				...
				scrollPosition = startPos
					+ (finalPosition - startPos) * easeProgress;
          ...
          {#each displayEntries as entry, i}
            ...
            {#if rarity !== 'gold'}
              ...
              >
                ...
                <LevelCard
                  {...toLevelCardProps(entry.level, list === 'fl' ? 'fl' : 'dl')}
                  type={list === 'fl' ? 'fl' : 'dl'}
                />
                <div
                  class="item-min-progress"
                  style="color: {getRarityColor(rarity)}; border-color: {getRarityColor(rarity)}"
                >
                  {entry.minProgress}%
                </div>
              </div>
  ...
  {#if hasOpened && wonLevel}
    <div class="result">
      ...
      <div class="result-content">
        <div
          class="rarity-border"
          style="border-color: {getRarityColor(getRarity(wonLevel))}"
        >
          <div
            class="item-glow"
            style="background: {getRarityColor(getRarity(wonLevel))}"
          >
          </div>
          <LevelCard
            {...toLevelCardProps(wonLevel, list === 'fl' ? 'fl' : 'dl')}
            type={list === 'fl' ? 'fl' : 'dl'}
          />
        </div>
        <div class="min-progress-display">
          <p>
            Minimum Progress: <strong
              style="color: {getRarityColor(getRarity(wonLevel))}"
            >{minProgress}%</strong>
          </p>
        </div>
      </div>
